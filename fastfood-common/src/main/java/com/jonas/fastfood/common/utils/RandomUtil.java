...
public class RandomUtil {
     ...
     * @return
     */
    public static String getDefaultDigitRandom() {
        String returnRet = "";
        try {
            setCharset("0-9");//指定字符内容为"0"-"z"，当然也可以是别的，比如"a-z"或"A-Z"

            setLength("6");//指定字符长度
            generateRandomObject();//生成随机数
            returnRet = getRandom();//得到随机数
        } catch (Exception e) {
            e.printStackTrace();
        }
        return returnRet;
    ...
    public static String createInviteCode() {
        String returnRet = "";
        try {
            setCharset("1-9A-Z");//指定字符内容为"0"-"z"，当然也可以是别的，比如"a-z"或"A-Z"
...
//		RandomUtil.setCharset("1-9A-Z");//指定字符内容为"0"-"z"，当然也可以是别的，比如"a-z"或"A-Z" 
//		RandomUtil.setCharset("0-9");
//		RandomUtil.setLength("6");//指定字符长度 
//		RandomUtil.generateRandomObject();//生成随机数 

//		String str = RandomUtil.getRandom();//得到随机数 
        for (int i = 0; i < 100; i++) {
            String randCode = RandomUtil.getDefaultDigitRandom();
            System.out.println(randCode);//输出结果：PE68P<HD]tnmj0Fp8i0R
        }
